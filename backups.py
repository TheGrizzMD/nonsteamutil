...
from pysteam import paths as pypaths
import shutil
...
#    shortcuts.write_shortcuts(backup_path, shortcuts.get_shortcuts(user))
    ...
    path_to_vdf = pypaths.shortcuts_path(user)
#    config_path = pypaths.user_config_directory(user)
    blank_shortcut = "shortcuts_blank.vdf"
    if os.path.exists(path_to_vdf):
        shutil.copy(path_to_vdf, backup_path)
    else:
        print("Shortcut file not found, creating blank shortcuts file")
        shutil.copy(blank_shortcut, path_to_vdf)
